...
public class ReportsSqsAdapter implements ReportsRepository {
    ...
    @Override
    public Mono<String> sendMessageReports(Double message) {
        return Mono.fromCallable(() -> objectMapper.writeValueAsString(message))
                .doOnNext(body -> log.info("[NOTIF->SQS] queue={} payload={}", properties.reportsQueueUrl(), body))
                .flatMap(body -> publisher.send(properties.reportsQueueUrl(), body));
    }
}
